...
public abstract class AbstractMongoDBComon {
     ...
     * @return 생성결과
     */
    protected boolean createCollection(String key, String index) {

        String[] indexArr = {index};

        return createCollection(key, indexArr);
    }
    ...
    protected boolean createCollection(String key, String[] index) {
        ...
        if (!mongodb.collectionExists(key)) {
            ...
            if (index.length > 0) {
                ...
                mongodb.createCollection(key).createIndex(Indexes.ascending(index));

            } else {

                // 인덱스가 없으면 인덱스 없이 컬렉션 생성
                mongodb.createCollection(key);
    ...
    @return 삭제결과
    */
    protected boolean dropCollection(String key) {
        boolean res = false;

        if (mongodb.collectionExists(key)) {
            mongodb.dropCollection(key);
            res = true;
        }
