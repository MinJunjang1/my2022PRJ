...
<div class="container body"  >
    ...
    <div class="card my-4">
        ...
        <div class="card-body">
                        ...
                        <div class="card-body">
                            <form name="comment-form" action="/board/recoin" method="post">
                                <div class="form-group">
                                    <input type="hidden"  name="bno" value="<c:out value="${pageInfo.bno}"/>" />
                                    <input type="hidden" name="comet_seq" value="${clist.comet_seq}" />
                                    <input type="hidden" id="recowriter" name="recowriter" value="${memberDTO.user_id}" />
                                    <textarea  id="reco_content" name="reco_content" class="form-control" rows="1"></textarea>
                                </div>
                                <button type="button" name="chatbtn" class="btn btn-primary" style="float: right;" onclick="">저장</button>
            ...
            <div class="divTableRow">
                <input type="hidden" value="${rlist.reco_seq}">
                <div class="divTableCell">대댓글 작성자 : <c:out value="${rlist.recowriter}"/> 작성시간 :

                    <fmt:formatDate pattern="yyyy/MM/dd" value="${rlist.recoregdate}"/>
                    <c:if test="${  rlist.recowriter ==  memberDTO.user_id }">
                        <a id="delete_btn"  style="float: right; text-align: right;" onclick="deletec(<c:out value="${rlist.reco_seq}"/>)">삭제</a>
                    </c:if>
